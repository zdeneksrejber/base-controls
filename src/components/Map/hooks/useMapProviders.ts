...
export const useMapProviders = (props: IUseMapProviders) => {
    ...
    const { MapProviderId, EnableProviderSwitching, DefaultMapProviderId } = parameters;
    //the picker is on and the map opens on the keyless default unless the maker says otherwise
    const letUserSwitch = EnableProviderSwitching?.raw !== false;
    const defaultVendorId = DefaultMapProviderId?.raw || DEFAULT_MAP_VENDOR_ID;
    ...
    const requestedProviderId = MapProviderId?.raw || DefaultMapProviderId?.raw || undefined;
