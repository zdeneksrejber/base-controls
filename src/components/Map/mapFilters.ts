 ...
 * the schema allows.
 *
 * @param records Records the map has loaded.
 * @param attributes Attribute paths to build facets for, in the order they should appear.
 * @param columns Dataset columns, for their display names.
 * @param maxOptions Distinct values a facet offers before the rest are dropped.
 * @returns One facet per attribute that has at least one value, most common value first.
 */
 ...
 *
 * @param selection Values picked per attribute.
 * @returns `true` when no attribute has a value picked.
 */
...
/**
 * Keeps the records matching the picked values.
 *
 * Values picked within one attribute widen the result, attributes narrow it - which is what a person clicking
 * through facets expects: "depots or stores, in Brno".
 *
 * @param records Records to filter.
 * @param selection Values picked per attribute.
 * @returns The matching records, in the order they came.
 */
...
/**
 * Turns the picked values into a filter for the bound dataset, so every control sharing it follows.
 *
 * Values within an attribute become an `In`, and the attributes are combined with `And`. A provider that
 * does not implement `In` for a given data type - the in-memory one used by demos, for instance - is why
 * the pin mode above exists.
 *
 * @param selection Values picked per attribute.
 * @returns The expression, or `null` when nothing is picked.
 */
 ...
 * @param attribute Attribute the value belongs to.
