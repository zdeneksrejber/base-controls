...
import { IMapAddressAttributeNames, parseMapAddressAttributeNames } from "./internal/address";
...
export const Map = (props: IMap) => {
    ...
    //the attribute the map geo-codes from is kept in step with a moved pin unless the mapping says otherwise
    const addressAttributes = useMemo<IMapAddressAttributeNames>(() => ({
        ...(addressAttribute ? { text: addressAttribute } : {}),
        ...parseMapAddressAttributeNames(AddressAttributeNames?.raw)
    }), [addressAttribute, AddressAttributeNames?.raw]);
    ...
    const drawnLocations = useMapClusters({
        ...
        options: props.clusteringOptions
    });
