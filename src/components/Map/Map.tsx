...
const MapBody = (props: IMapBodyProps) => {
    ...
    const { records: loadedRecords, isLoading: isLoadingAllPages, loadedCount, isTruncated } = useMapRecords({
        dataset,
        loading: LoadAllPages?.raw === true ? 'all' : 'page',
    ...
    const overlays = useMapOverlayItems(useMemo(() => [
        ...moduleOverlays,
        { position: 'top-left' as const, order: MAP_OVERLAY_ORDER.status, element: <MapStatus {...status} theme={theme} /> },
        ...(options.length > 1
            ? [{
                position: 'top-right' as const,
                order: MAP_OVERLAY_ORDER.providerPicker,
                element: <MapProviderPicker
                    options={options}
            ...
            : [])
    ], [moduleOverlays, status, theme, options, selectedId, labels, onPickProvider]));
