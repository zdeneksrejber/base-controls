 ...
 *
 * Returns nothing outside a host that provides `Xrm`, so a control running in Storybook, a test, or a
 * standalone app degrades to whatever it was going to do without the web resource rather than throwing.
 *
 * @param webResourceName Name of the web resource, as a maker typed it.
 * @returns An absolute url, or `undefined` when there is no host to ask.
 */
 ...
 * @param signal Aborts the fetch when the control stops caring.
 * @returns The text, or `undefined` when the resource cannot be resolved or read.
 */
