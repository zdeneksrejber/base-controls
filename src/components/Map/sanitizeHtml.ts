...
/**
 * Cleans HTML before the control puts it on the page.
 *
 * The legend is the one place the Map renders markup it did not write, and that markup comes from a manifest
 * property or a web resource - both of which a maker edits, and neither of which is reviewed. Scripts, event
 * handlers, forms and anything that can load or submit are removed; formatting, tables, images and inline
 * SVG survive, because that is what a legend is made of.
 *
 * @param html Markup to clean.
 * @returns Markup safe to insert, or an empty string when there was nothing usable.
 */
