...
/**
 * Works out what activating a record's pin should do.
 *
 * The same matching as the pin rules, so "depots open an Adaptive Card, everything else runs a function" is
 * expressed the same way as "depots are red".
 *
 * @param record Record whose pin was activated.
 * @param rules Card rules, in the order they should be tried.
 * @param fallback Card to use when no rule applies.
 ...
 *
 * `statecode@OData.Community.Display.V1.FormattedValue` becomes `statecode_label`, because an Adaptive Cards
 * binding expression cannot address a key containing `@` or `.`. The legacy MapPicker did this in place; a
 * copy is returned here instead, so rendering a card never mutates the dataset.
 *
 * @param data Raw record data, or a nested object reached while walking it.
 * @returns A copy with every annotation renamed, at every depth.
 */
