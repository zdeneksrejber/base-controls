...
import { getAddressValues, hasAddressAttributes, parseMapAddressAttributeNames } from './address';
...
describe('parseMapAddressAttributeNames', () => {
    it('reads the mapping a maker typed', () => {
        expect(parseMapAddressAttributeNames(JSON.stringify(ATTRIBUTE_NAMES))).toEqual(ATTRIBUTE_NAMES);
    });

    it('reads nothing from an empty or malformed parameter', () => {
        expect(parseMapAddressAttributeNames(undefined)).toEqual({});
        expect(parseMapAddressAttributeNames('   ')).toEqual({});
        expect(parseMapAddressAttributeNames('{ not json')).toEqual({});
        expect(parseMapAddressAttributeNames('["cds_city"]')).toEqual({});
    });
});

describe('getAddressValues', () => {
    it('maps every component onto the attribute it is bound to', () => {
        expect(getAddressValues(ADDRESS, ATTRIBUTE_NAMES)).toEqual({
    ...
    it('clears a component the service could not resolve, rather than leaving the old value', () => {
        const values = getAddressValues({ locality: 'Praha' }, ATTRIBUTE_NAMES);
