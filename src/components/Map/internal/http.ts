...
/**
 * Builds a queue that serializes calls and keeps a minimum gap between them. Public geocoding services
 * publish a rate limit, and exceeding it gets an origin blocked rather than throttled - so the limit
...
/**
 * Runs work over a list with a bounded number of calls in flight. A view can name hundreds of addresses to
 * resolve, and firing them all at once is what gets an origin rate limited - so the work is spread rather
 * than batched. A rejected item resolves to `undefined` rather than stopping the run.
 */
