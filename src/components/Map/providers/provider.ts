...
import { IMapCoordinates, IMapViewport } from '../internal/viewport';
...
export interface IMapProviderOption {
    ...
    label?: string;
    /**
     * Rendered as handed over, so keep the same component identity across renders - `useMapProviderCache`
     * does that per configuration key. A fresh identity remounts the map.
     */
