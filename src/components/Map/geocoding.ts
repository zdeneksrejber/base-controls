 ...
 * @returns The description, or an empty string when there is nothing to describe.
 */
 ...
 *
 * @param coordinates Point to key.
 * @returns A stable key, precise to roughly a tenth of a metre.
 */
const getCoordinatesKey = (coordinates: IMapCoordinates): string =>
    `${coordinates.latitude.toFixed(6)},${coordinates.longitude.toFixed(6)}`;

/**
 * Wraps a geocoder so the same lookup is only ever made once.
 *
 * In flight calls are shared rather than duplicated, which matters when a page of records all carry the
 * same address - and a failed call is forgotten, so a transient error does not become a permanent one.
 *
 * @param geocoder Geocoder to wrap.
 * @param maxEntries Lookups to remember before discarding the ones used longest ago.
 * @returns A geocoder with the same behaviour and no repeated calls.
 */
