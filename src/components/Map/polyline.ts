...
/**
 * Undoes the zigzag encoding both formats apply, which maps signed numbers onto unsigned ones.
 *
 * Arithmetic rather than bit operations throughout, because a coordinate scaled to seven decimal places
 * overflows the 32 bits JavaScript bit operators work in.
 *
 * @param value Unsigned zigzag encoded value.
 * @returns The signed value it stands for.
 */
 ...
 * @param toCharValue Maps one character to the six or five bits it carries.
 ...
 *
 * Used by the Routes API and by every service that adopted the same format.
 *
 * @param encoded Encoded polyline.
 * @param precision Decimal places the coordinates were scaled to. Google uses five.
 * @returns The coordinates, in order. An empty string decodes to no coordinates.
 */
...
/**
 * Decodes a HERE flexible polyline into coordinates.
 *
 * The format carries its own precision in a header, and optionally a third dimension - elevation, say -
 * which is read past and dropped, since the control draws in two dimensions.
 *
 * @param encoded Encoded flexible polyline.
 * @returns The coordinates, in order. An empty string decodes to no coordinates.
 */
