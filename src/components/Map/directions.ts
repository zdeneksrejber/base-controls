...
/**
 * Splits a run of stops into legs no longer than a service accepts.
 *
 * Legs overlap by one stop, so putting their paths back together leaves no gap at the joins.
 *
 * @param stops Stops in order.
 * @param maxStops Most stops one call may carry. Anything below two is treated as two.
 * @returns The legs, each at least two stops long. Fewer than two stops produces no legs.
 */
...
/**
 * Routes a whole run of stops, splitting it across calls when the service caps how many it takes.
 *
 * @param directions Directions service to route with.
 * @param stops Stops in order.
 * @param options Language and abort signal.
 * @returns The joined path, or `null` when any leg could not be routed.
 */
