...
`EnableClustering` is on by default; the `clusteringOptions` prop overrides the radius, the zoom ceiling and
how many members a group lists.
...
Where `AddressAttributeNames` maps any address component to an attribute, either gesture also reverse
geo-codes the point and writes the components back — which is what makes a map click a usable way to fill in
an address. The parameter is a JSON object keyed by the components of `IAddress`:

```json
{ "text": "address1_composite", "country": "address1_country", "administrativeArea": "address1_stateorprovince",
  "locality": "address1_city", "subLocality": "address1_line3", "street": "address1_line1",
  "streetNumber": "address1_line2", "postalCode": "address1_postalcode" }
```

`FullAddressAttributeName`, the attribute the map geo-codes *from*, is written as `text` too unless the mapping
names another attribute for it, so the address a pin was placed by stays in step with where it was moved to.
...
`EnableProviderSwitching`, `DefaultMapProviderId` and the `<Vendor>ApiKey` properties are resolved by the
control itself. On the manifest every vendor is simply a provider a maker can name by id.

- A provider is offered as soon as its api key has a value. OpenStreetMap is keyless, so it is always offered.
- `EnableProviderSwitching` (**on by default**) offers that list through the picker. Off, the control draws
  `DefaultMapProviderId` alone, with no picker.
- `DefaultMapProviderId` (**`leaflet` by default**) is the provider the map opens with. An id with no
  configured vendor behind it falls back to `leaflet` and warns rather than failing silently.
...
| `clusteringOptions` | `IMapClusteringOptions` | Grouping radius, zoom ceiling, members a group lists. |
| `viewportOptions` | `IMapViewportOptions` | Fallback centre and the zoom levels the viewport is derived with. |

### `parameters` — `IMapParameters`

`Dataset` is a `data-set`; everything else is a static manifest input, and every one but `Dataset`,
`LatitudeAttributeName` and `LongitudeAttributeName` is optional. Anything a manifest cannot express is a prop
on the control instead.

| Group | Parameters |
|-------|------------|
| **Binding** | `Dataset`, `LatitudeAttributeName`, `LongitudeAttributeName`, `EnableAttributeLinking` |
| **Data** | `PinLoading`, `MaxRecords`, `FullAddressAttributeName`, `MaxGeocodingRequests`, `PersistGeocodedCoordinates`, `EnableClustering`, `FilterAttributeNames`, `FilterMode`, `EnableSearch`, `EnableAddressSearch` |
...
| **Editing** | `EnablePinDragging`, `EnablePinCreation`, `PrefillUserLocation`, `AddressAttributeNames` |
| **Chrome** | `Legend`, `LegendWebResourceName`, `ShowPointsOfInterest` |
| **Providers** | `EnableProviderSwitching`, `DefaultMapProviderId`, `MapProviderId`, `HereApiKey`, `MapyApiKey`, `GoogleApiKey`, `<Vendor>ApiKey` |
...
<property name="AddressAttributeNames" display-name-key="Address attributes (JSON)" of-type="Multiple" usage="input" required="false" />
...
<property name="EnableProviderSwitching" display-name-key="Let the user switch provider" of-type="TwoOptions" usage="input" required="false" />
<property name="DefaultMapProviderId" display-name-key="Default map provider" of-type="SingleLine.Text" usage="input" required="false" />
