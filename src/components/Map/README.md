...
| `LoadAllPages` | Whether every page of the view is drawn, on a copy of the dataset. Off by default. |
| `MaxRecords` | Records to load before stopping, while `LoadAllPages` is on. Defaults to 50 000. |
| `PinRules` | Pin rules as a JSON array; first match wins. Same shape as the legacy MapPicker's `pinIcons`. |
| `ShowPointsOfInterest` | Whether the provider's own points of interest are drawn. Off by default. |
| `DefaultProvider` | Provider the map opens with. Defaults to `leaflet`. |
| `EnableProviderSwitching` | Whether the picker offers every configured provider. On by default. |
| `MapProviderId` | The end user's pick, fed back to make it stick. |
...
| `MapProviderId` | The provider the end user picked. |

## Modules

| Key | Built by | What it adds |
|---|---|---|
| `filter` | `createFilterModule({ attributes, scope })` | A panel of the values the records hold, to narrow the pins by |
...
| `search` | `createSearchModule({ places })` | Quick find over the records, and places from the geo-coding service |
| `legend` | `createLegendModule({ html, webResourceName })` | A legend over the map |
...
through its `labels` option), its UI, its tests and its story.
...
| `useStatus` | the view context | a message for the status pill | geocoding, editing |
...
component folders, and a story under `Map/Modules`. Add its key to `IMapModules` and its place in
`MAP_MODULE_ORDER`.
...
`modules/cards/map-card/adaptive-card`, because `adaptivecards` is optional.

A provider of your own is an `IMapProviderDefinition`: an `id`, a `label`, the `apiKeyParameterName` its
key is read from, and `createProvider` returning a component that takes `IMapProviderProps`. `provider.ts`
documents every prop a provider receives; the shipped Leaflet provider is the reference implementation.
